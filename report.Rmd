---
title: "COVID-19 Vaccines and Social Vulnerability Index"
...
Throughout the United States there are wildly varying COVID-19 vaccination rates. While vaccine hesitance is often discussed as a chief reason for people not getting the vaccine, simple lack of access to vaccines was also discussed, especially in the first few months that the vaccine was available. Vaccines could be less accessible for populations that live in less metropolitan areas, where it is more difficult to find a vaccine clinic, or in populations that are considered more vulnerable, where it may be more difficult for people to get the vaccine for a variety of reasons. This project attempts to see if there is some relationship between county-level COVID-19 vaccination rates and the CDC's Social Vulnerability Index (SVI), which using multiple factors to attempt to identify populations that may be more at risk during public health emergencies.
...
Figures 3 and 4 compare the COVID-19 vaccination rate of the average US county to the average counties of each SVI quartile. Group A has an SVI percentile of 0 to 25, Group B has a percentile of 25 to 50, Group C has a percentile of 50 to 75, and Group D has a percentile of 75 to 100. It appears that Group A has the highest vaccination rate, with a rate of `r svi1`%, with the rate decreasing as we go to less vulnerable groups. The change from August 1 score has Group D - the most vulnerable group - with the highest change at an increase of `r svi2` percentage points, and the other three groups about even.
...
These plots also show at least one potential issue with the vaccination data. At certain points in time, the vaccination rates decrease. A closer look at the county-by-county vaccination data, available through the R Shiny app, shows that this is the case in multiple counties' raw data. Also, some counties, such as those in Texas or Hawai'i, only begin reporting data in October, which potentially inaccurate rates. To try to help mitigate the effects of these problems, only counties with a non-missing change from August 1 greater or equal to 0 were included in the analysis. Also, all data points with a vaccination rate of 0 are removed and assumed to be missing. The final data set for analysis contains `r tot$n` counties. Outliers were then removed for the linear regression and PCA analyses. Outliers were those that were less than Q1 - (1.5 IQR) and greater than Q3 + (1.5 IQR), where Q1 and Q3 were the 1st and 3rd quartile values, respectively, and IQR is the interquartile range, calculated as Q3 - Q1. Thus we have `r tot1$n` counties for the rate on November 31 analysis, and `r tot2$n` counties for the change from August 1 analysis.

## Vaccinations vs. Social Vulnerability Index

A general idea of the relationship between vaccination rate the SVI can be seen in the below scatterplots (Figures 5 and 6), which show the rate and the change from August 1 on November 30. There appears to be a slight decrease in vaccination rate as the SVI score gets higher (indicating more vulnerable counties), and perhpas a slight increase in change from August 1 as the SVI score gets higher, but any slope is too slight and has too much variation to say much from these figures.

\setcounter{figure}{4}

![COVID-19 Vaccination Rate vs. Overall SVI]("~/project/figures/themes.png")

![Change in COVID-19 Vaccination Rate from August 1 vs. Overall SVI]("~/project/figures/themes_aug1.png")
...
The below table 4 shows the result of the model run with vaccine rate on November 31 as the outocme and the individual SVI variables as the covariates. The adjusted R squared for this model is `r rsq1_1` Table 5 shows the result of the model run with the change in vaccine rate from August 1 as the outcome and the theme scores as the covariates. The adjusted R squared for this model is `r rsq2_1`.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
kable(coef(summary(lm1_1)), caption="Linear regression results of vaccine rate vs. SVI variables")
kable(coef(summary(lm2_1)), caption="Linear regression results of change in vaccine rate from August 1 vs. SVI variables")
```

Again, at the 0.05 alpha level, many of these covariates have significant p-values for both outcomes. The R squared values are again fairly low, indicating a poor fit.
...
We can try to visualize any trends by using a principal component analysis of the SVI data, and graphing vaccination rates on top of the first two principal components. The below two plots were created by performing principal component analysis on the SVI themes, percent unemployed, and metro status. For the first plot, the first two principal components cover `r var1`% of the variance. For the second plot, the first two principal components cover `r var3`% of the variance. Note that these values should be close to identical or identical; however, any differences comes from the fact that outliers for the vaccination rate may not be the same as the outliers for the change from August 1st, so the two data sets may contain different counties.

![PCA with SVI Themes with Vaccination Rate Overlay]("~/project/figures/pca1_rate.png")

![PCA with SVI Themes with Change in Vaccination Rate from August 1st Overlay]("~/project/figures/pca1_chg.png")

The below two plots were then created by performing principal component analysis on the individual SVI variables, percent unemployed, and metro status. For the first plot, the first two principal components cover `r var2`% of the variance. For the second plot, the first two principal components cover `r var4`% of the variance.

![PCA with SVI Variables with Vaccination Rate Overaly]("~/project/figures/pca2_rate.png")

![PCA with SVI Variables with Change in Vaccination Rate from August 1st Overlay]("~/project/figures/pca2_chg.png")
...
Overall, it is difficult to comfortably make any conclusions about the relationships between SVI variables and COVID-19 vaccination rates in counties in the United States. It does seem that less vulnerable counties may generally have higher COVID vaccination rates, but the data is too varied to really draw any conclusions. The best model was the themes linear regression model for vaccination rate. With an R squared of `r rsq1` it had significant p-values for socioeconomic status, minority status and language, housing type and transportation, uninsured, and metro status. A lower socioeconomic status score (i.e. higher income, less unemployment, etc.), and a higher minority status and housing score (i.e. a higher percentage of minorities and higher crowding, multi-unit structures, group housing, etc.) were associated with a higher vaccination rate on November 31. Similarly, a lower percent uninsured was associated with a higher vaccination score, and metro counties tended to have a higher vaccination status.
